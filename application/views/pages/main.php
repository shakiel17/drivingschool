                        ...
                        <div class="d-flex justify-content-between">
                              <table class="table border mb-4 mt-4">
                                ...
                                <tbody>
                                  ...
                                  $query=$this->School_model->db->query("SELECT s.id,i.fullname,c.description,s.datearray,s.starttime,s.endtime,s.status,s.regno FROM schedule s INNER JOIN enrollee e ON e.regno=s.regno INNER JOIN instructor i ON i.id=s.instructor_id INNER JOIN cars c ON c.id=s.car_id INNER JOIN user cs ON cs.customer_no=e.customer_no WHERE cs.username='$username' ORDER BY s.datearray ASC");
                                  $result=$query->result_array();
                                  foreach($result as $item){
                                    $report=$this->School_model->getProgressReport($item['id']);
                                    $remarks="";
                                    if($report){
                                    ...
                                    echo "<tr>";
                                      ...
                                      echo "<td>".date('m/d/Y',strtotime($item['datearray']))." ".date('h:i A',strtotime($item['starttime']))."-".date('h:i A',strtotime($item['endtime']))."</td>";
